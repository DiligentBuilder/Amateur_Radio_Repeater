          ...
          if (strncmp(old_password_entry_chars, old_password_chars, 6) == 0) {
            ...
            while (1) {
              ...
              if (bufferString.indexOf("*") != -1) {
                ...
                for (i = 0; i < bufferString.indexOf("*"); i++) {
                  speak(bufferString.charAt(i));
                }
                ...
                for (i = 0; i < bufferString.indexOf("*"); i++) {
                  if (bufferString.charAt(i) == '1') {
                    morse_code_string.append(".");
                  }

                  if (bufferString.charAt(i) == '2') {
                    morse_code_string.append("-");
                  }

                  if (bufferString.charAt(i) == '3') {
                    morse_code_string.append(" ");
                  }
                }

                // once we have the Morse code string, we will play the Morse code back through the PWM pin on the ATMega
                transmitMorseCode(morse_code_string);
